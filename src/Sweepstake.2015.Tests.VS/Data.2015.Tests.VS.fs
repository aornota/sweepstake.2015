...
[<TestClass>]
type ``Data 2015 Tests`` () =

    [<TestMethod>]
    member x.``Verify Player Team for all 2015 Squads`` () =
    ...
    [<TestMethod>]
    member x.``Verify total non-withdrawn Players for all 2015 Squads`` () =
        let failed = ``Get Squads with incorrect total non-withdrawn Player counts`` squads 31
    ...
    [<TestMethod>]
    member x.``Verify MatchEvent Teams for all 2015 Matches`` () =
    ...
    [<TestMethod>]
    member x.``Verify TeamScores for all 2015 Matches`` () =
    ...
    [<TestMethod>]
    member x.``Verify total Conversion attempts for all 2015 Matches`` () =
